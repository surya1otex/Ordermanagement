...
<?php
 if(isset($_POST['add_supplier'])){
   $req_fields = array('supplier-name','adress','country', 'state', 'city','phone', 'email', 'website' );
   ...
   if(empty($errors)){
     ...
     $s_ph   =  remove_junk($db->escape($_POST['phone']));
     $s_email = remove_junk($db->escape($_POST['email']));
     $s_web   = remove_junk($db->escape($_POST['website']));
     //$date    = make_date();
     $query  = "INSERT INTO suppliers (";
     $query .=" supp_name,address,country,state,city,phone,email,url";
     $query .=") VALUES (";
     $query .=" '{$s_name}', '{$s_add}', '{$s_cou}', '{$s_sta}', '{$s_cit}', '{$s_ph}', '{$s_email}','{$s_web}'";
     $query .=")";
     $query .=" ON DUPLICATE KEY UPDATE supp_name='{$s_name}'";
     if($db->query($query)){
       $session->msg('s',"Supplier added ");
       redirect('add_sales_person.php', false);
     } else {
  ...
  <div class="col-md-8">
      <div class="panel panel-default">
        ...
        <div class="panel-body">
         <div class="col-md-12">
          <form method="post" action="add_sales_person.php" class="clearfix">
              <div class="form-group">
                <div class="input-group">
                  <span class="input-group-addon">
                   <i class="glyphicon glyphicon-th-large"></i>
                  </span>
                  <input type="text" class="form-control" name="supplier-name" placeholder="Name">
               </div>
              </div>
              <div class="form-group">
                <div class="input-group">
